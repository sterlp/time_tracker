...
class StartAndStopWidget extends StatelessWidget {
  ...
  const StartAndStopWidget(
    this._showStart,
    this._workTime,
    this._targetWorkTime,
    this._startPress, {
    super.key,
  });
  ...
  Widget _buildDailyProgress(Size size, MaterialColor color) {
    ...
    final progress = _targetWorkTime.inSeconds > 0
        ? _workTime.inSeconds / _targetWorkTime.inSeconds
        : 100.0;
    return SizedBox(
  ...
  Widget _buildStartButton(
    ...
    Widget text;
    if (_showStart) {
      text = Text('Start', style: Theme.of(context).textTheme.headlineMedium);
    } else {
      text = Text('Stopp', style: Theme.of(context).textTheme.headlineMedium);
    }
